...
public class BitmapUtil {
    ...
    /**
     * 但是在android中，当我们将手机本地照片decod为bitmap后，bitmap对象中是没有这些Exif信息的，
     * 而一般情况下用户拍照后将照片上传是肯定不会让上传原图的，都会对图片进行适当压缩。
     * 无论是Bitmap.compress还是BitmapFactory inSampleSize压缩，Exif信息都会丢失，
     * 这显然不是我们想要的结果，我们需要将原图的Exif信息写入新的压缩后的图片中，那么怎么做呢，
     * 其实Exif信息是存储在于图片的文件头信息中，它拥有自己规定好的标识符，
     * 我们可以按照其标准使用OutputStream写入，但是这样当然非常的麻烦，
     * 而android中已经为我们提供了便捷的方法，那就是使用ExifInterface。
     * @param oldFilePath
     * @param newFilePath
     * @throws Exception
     */
