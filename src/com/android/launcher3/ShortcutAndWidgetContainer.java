...
public class ShortcutAndWidgetContainer extends ViewGroup {
    ...
    int getCellContentHeight() {
        final LauncherAppState app = LauncherAppState.getInstance();
        final DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
        if (mIsHotseatLayout && getResources().getBoolean(R.bool.hotseat_title_show)){
    ...
    public void measureChild(View child) {
        ...
        if (!lp.isFullscreen) {
            ...
            }  else if ((child.getTag() != null && child.getTag() instanceof CustomWidgetInfo)) {
				CustomWidgetInfo customWidgetInfo = (CustomWidgetInfo) child.getTag();
				// if the CustomWidget is 1x1, then use the padding of the normal icon
				if (customWidgetInfo.spanX == 1 && customWidgetInfo.spanY == 1) {
					int cHeight = getCellContentHeight();
					int cellPaddingY = (int) Math.max(0, ((lp.height - cHeight) / 2f));
					int cellPaddingX = (int) (grid.edgeMarginPx / 2f);
					child.setPadding(cellPaddingX, cellPaddingY, cellPaddingX, 0);
				} else {
					// other CustomWidgets have their own padding, so skip
				}
			} else {
