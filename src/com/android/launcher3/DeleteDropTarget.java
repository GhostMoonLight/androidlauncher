...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    protected void onFinishInflate() {
        ...
        mRemoveDrawable = (TransitionDrawable) r.getDrawable(R.drawable.uninstall_target_selector);
    ...
    private boolean isDragSourceWorkspaceOrFolder(DragObject d) {
        return (d.dragSource instanceof Workspace) || (d.dragSource instanceof FolderBase);
    }
    ...
    private boolean isWorkspaceCustomWidget(DragObject d) {
    	 return (d.dragSource instanceof Workspace) && (d.dragInfo instanceof CustomWidgetInfo);
    }
    //第一屏中不是快捷方式的icon
    private static boolean isWorksapceShortcut(Object info) {
    ...
    public boolean acceptDrop(DragObject d) {
        if (isSelfComponent(LauncherAppState.getInstance().getContext(), d.dragInfo))
        	return false;
        if (Workspace.isShowUninstallIcon){
        	return true;
        }
        return willAcceptDrop(d.dragInfo);
    }
    
    //判断是否该target能否接受当前的DragView
    public static boolean willAcceptDrop(Object info) {
    	if (isSelfComponent(LauncherAppState.getInstance().getContext(), info))
        	return false;
        ...
        if (info instanceof ItemInfo) {
            ...
            //第一屏中不是快捷方式的icon
            if (LauncherAppState.isDisableAllApps() && isWorksapceShortcut(info)) {
            	if (PackageUtil.canUninstall(LauncherAppState.getInstance().getContext(), (ShortcutInfo)info)){
            		return true;
            	}
                return false;
            } else {
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
    	boolean showIcon = true;    //显示删除图标，还是显示提示文字
        ...
        if (isSelfComponent(getContext(), info)) {
        	mActive = showIcon = false;
			...
			if (info instanceof ShortcutInfo) {
				if (LauncherAppState.isDisableAllApps()) {
					...
					if (shortcutInfo.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
						boolean canUninstall = PackageUtil.canUninstall(mLauncher, shortcutInfo);
						showIcon = canUninstall;
						mActive = canUninstall;
				...
				FolderInfo sFolderInfo = (FolderInfo) info;
				
				if (sFolderInfo.contents == null || sFolderInfo.contents.size() == 0) {
					showIcon = true;
					setText(R.string.delete_target_label);
				} else {
					mActive = showIcon = false;
	 ...
	 * 是否是自身组件
	 */
	public static boolean isSelfComponent(Context context, Object info) {
		
		if (info instanceof ShortcutInfo){
			ShortcutInfo sInfo = (ShortcutInfo) info;
			String strPkg = ItemInfo.getPackageName(sInfo.intent);
			if (context.getPackageName().equals(strPkg) || ("com.example.viewpagertest".equals(strPkg))) {
				//com.example.viewpagertest
				
				return true;
			}
		} else if (info instanceof CustomWidgetInfo){
			CustomWidgetInfo sInfo = (CustomWidgetInfo) info;
			if (sInfo.itemType == LauncherSettings.Favorites.ITEM_TYPE_CUSTOM_WIDGET_TIME_AND_WEATHER){
				return true;
			}
		}
    ...
    private void deferCompleteDropIfUninstalling(DragObject d) {
        ...
        if (isUninstallFromWorkspace(d)) {
            if (d.dragSource instanceof FolderBase) {
                ((FolderBase) d.dragSource).deferCompleteDropAfterUninstallActivity();
                ...
                if (mWaitingForUninstall) {
                    final Runnable checkIfUninstallWasSuccess = new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (dragSource instanceof FolderBase) {
                                ((FolderBase) dragSource).
...
//        	ShortcutInfo shortcutInfo = (ShortcutInfo) d.dragInfo;
        ...
        } else if (isWorkspaceCustomWidget(d)) {
        	LauncherModel.deleteItemFromDatabase(mLauncher, item);
        ...
        if (wasWaitingForUninstall && !mWaitingForUninstall) {
            if (d.dragSource instanceof FolderBase) {
                ((FolderBase) d.dragSource).onUninstallActivityReturned(false);
            } else if (d.dragSource instanceof Workspace) {
