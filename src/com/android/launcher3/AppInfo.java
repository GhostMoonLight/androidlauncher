...
public class AppInfo extends ItemInfo {
    ...
    static final int DOWNLOADED_FLAG = 1;   //非系统应用
    static final int UPDATED_SYSTEM_APP_FLAG = 2;  //可卸载更新的系统应用
     ...
     * @param info    LauncherActivityInfoCompat
     * @return   flags
     */
    private static int initFlags(LauncherActivityInfoCompat info) {
        int appFlags = info.getApplicationInfo().flags;
        int flags = 0;
        if ((appFlags & android.content.pm.ApplicationInfo.FLAG_SYSTEM) == 0) { //非系统应用
            flags |= DOWNLOADED_FLAG;

            if ((appFlags & android.content.pm.ApplicationInfo.FLAG_UPDATED_SYSTEM_APP) != 0) {
            	//可卸载更新的系统应用
                flags |= UPDATED_SYSTEM_APP_FLAG;  //该值为二进制11 即值为3
            }
        }
        return flags;
    }
