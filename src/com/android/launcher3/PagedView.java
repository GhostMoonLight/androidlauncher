...
import com.android.launcher3.utils.SettingInfo;
 ...
 * 
 *  TransitionEffect.setFromString(getWorkspace(), "立方体外部"); 设置切换动画
 */
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected int mChildCountOnLastLayout;   //上次布局中child的数量
    ...
    protected boolean mAllowOverScroll = false;   //workspace滑到边缘是否有阴影拉伸效果，为true时mEffectOverScroll置为false
    ...
    public boolean mUseMinScale = false;
    ...
    private boolean isLimit = false;    //是不是在第一屏或者最后一屏，只有在这两个情况继续往右滑或者往左滑时屏幕才需要循环
	...
	protected static final float TRANSITION_SCREEN_ROTATION = 22.5f;
	protected static float CAMERA_DISTANCE = 6500;
	
	protected boolean mEffectOverScroll = false;  //回弹效果
    private int mEffectOverX = 0;
    ...
    public PagedView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mSettingInfo = SettingInfo.getInstance(context);
        init();
    }
    ...
    public void scrollTo(int x, int y) {
        ...
        if(mSettingInfo.getScreenRount()){
			if (isXBeforeFirstPage) {   //x < 0
				isLimit = true;
				mOverScrollX = 0;
			} else if (isXAfterLastPage) {  //x > mMaxScrollX
				isLimit = true;
			...
			isXBeforeFirstPage = isRtl ? (x > mMaxScrollX+mEffectOverX) : (x < -mEffectOverX);
			isXAfterLastPage = isRtl ? (x < -mEffectOverX) : (x > (mMaxScrollX+mEffectOverX));
			if (isXBeforeFirstPage) {
				super.scrollTo(-mEffectOverX/4, y);
				if (mAllowOverScroll) {
					if (isRtl) {
				...
				super.scrollTo(mMaxScrollX+mEffectOverX/4, y);
				if (mAllowOverScroll) {
					if (isRtl) {
						overScroll(x);
					} else {
				...
				if (mEffectOverScroll && x < 0){
					super.scrollTo(getGetOverScrollX(x), y);
				}else if (mEffectOverScroll && x > mMaxScrollX){
					super.scrollTo(mMaxScrollX+getGetOverScrollX(x-mMaxScrollX), y);
				}else{
					super.scrollTo(x, y);
				}
	        }
    ...
    //PagedView自定义的LayoutParams,isFullScreenPage该孩子是不是全屏显示
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        final int delta = isRtl ? -1 : 1;  //每次的便增量
                 ...
                 if (child != null) {
                     ...
                     if (mFadeInAdjacentScreens && !isInOverscroll) {
                    	 ...
                    	 if (child instanceof CellLayout) {
 							((CellLayout) child).getChildrenLayout().setAlpha(alpha);
 						} else {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (pageCount > 0) {
            ...
            if (leftScreen != -1 && rightScreen != -1) {
                ...
                if (isLimit && mSettingInfo.getScreenRount()) {
					int offset = getWidth() * pageCount;
					if (getScrollX() >= mMaxScrollX) {
						drawChild(canvas, getPageAt(getChildCount() - 1), drawingTime);
						canvas.translate(+offset, 0);  //移动到当前布局的最右端，也就是最后边缘,画出第一个屏view
...
//        mAllowOverScroll = enable;
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (!mFreeScroll) {
                    ...
                    if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                    	...
                    	if(mCurrentPage<=0&&mSettingInfo.getScreenRount()){
    						if(deltaX < 0){
    							finalPage = mCurrentPage + 1;
    						} else{
    						finalPage = getChildCount()-1;		}				
    						snapToPageWithVelocity(finalPage, velocityX);
    					}else if (mCurrentPage>=(getChildCount()-1)&&mSettingInfo.getScreenRount()) {
    						finalPage=0;						
    						snapToPageWithVelocity(finalPage, velocityX);
    					}else {
    						snapToDestination();
    ...
    protected void snapToPage(int whichPage, int delta, int duration, boolean immediate,
        ...
        int offset = 0;
        if(isLimit&&mSettingInfo.getScreenRount()){
			offset=getWidth()*getChildCount();
			if(delta>0){
				delta=-(getWidth()+mUnboundedScrollX);
    ...
    public void setEffectOverScroll(boolean effectOverScroll){
		...
		if (effectOverScroll){
			mEffectOverX = getResources().getDimensionPixelOffset(R.dimen.paged_scroll_over);
		}else{
			mEffectOverX = 0;
		}
	}
    ...
    protected static abstract class TransitionEffect {
		...
		public static void setFromString(PagedView pagedView, String effect) {
			String[] transitions = pagedView.getResources().getStringArray(R.array.transition_list);
			
			if (transitions.length > 1 && transitions[1].equals(effect)) {
				pagedView.setTransitionEffect(new Zoom(pagedView, true));
			} else if (transitions.length > 2 && transitions[2].equals(effect)) {
				pagedView.setTransitionEffect(new Zoom(pagedView, false));
			} else if (transitions.length > 5 && transitions[5].equals(effect)) {
				pagedView.setTransitionEffect(new Cube(pagedView, true));
			} else if (transitions.length > 6 && transitions[6].equals(effect)) {
				pagedView.setTransitionEffect(new Cube(pagedView, false));
			} else if (transitions.length > 3 && transitions[3].equals(effect)) {
				pagedView.setTransitionEffect(new Rotate(pagedView, true));
			} else if (transitions.length > 4 && transitions[4].equals(effect)) {
				pagedView.setTransitionEffect(new Rotate(pagedView, false));
			} else if (transitions.length > 7 && transitions[7].equals(effect)) {
				pagedView.setTransitionEffect(new Stack(pagedView));
			} else if (transitions.length > 8 && transitions[8].equals(effect)) {
				pagedView.setTransitionEffect(new Accordion(pagedView));
			} else if (transitions.length > 9 && transitions[9].equals(effect)) {
				pagedView.setTransitionEffect(new Flip(pagedView));
			} else if (transitions.length > 10 && transitions[10].equals(effect)) {
				pagedView.setTransitionEffect(new Cylinder(pagedView, true));
			} else if (transitions.length > 11 && transitions[11].equals(effect)) {
				pagedView.setTransitionEffect(new Cylinder(pagedView, false));
			} else if (transitions.length > 12 && transitions[12].equals(effect)) {
				pagedView.setTransitionEffect(new Carousel(pagedView));
			} else if (transitions.length > 13 && transitions[13].equals(effect)) {
		...
		public static class Zoom extends TransitionEffect {
			...
			public Zoom(PagedView pagedView, boolean in) {
				super(pagedView, in ? pagedView.getResources().getStringArray(R.array.transition_list)[1] : 
					pagedView.getResources().getStringArray(R.array.transition_list)[2]);
				mIn = in;
			}

			@Override
			public void screenScrolled(View v, int i, float scrollProgress) {
				float scale = 1.0f + (mIn ? -0.4f : 0.2f) * Math.abs(scrollProgress);
		...
		public static class Rotate extends TransitionEffect {
			...
			public Rotate(PagedView pagedView, boolean up) {
				super(pagedView, up ? pagedView.getResources().getStringArray(R.array.transition_list)[3] : 
					pagedView.getResources().getStringArray(R.array.transition_list)[4]);
				mUp = up;
			}
			...
			public void screenScrolled(View v, int i, float scrollProgress) {
				...
				float rotatePoint = (v.getMeasuredWidth() * 0.5f) / (float) Math.tan(Math.toRadians((double) (TRANSITION_SCREEN_ROTATION * 0.5f)));
		...
		public static class Cube extends TransitionEffect {
			...
			public Cube(PagedView pagedView, boolean in) {
				super(pagedView, in ? pagedView.getResources().getStringArray(R.array.transition_list)[5] : 
					pagedView.getResources().getStringArray(R.array.transition_list)[6]);
				mIn = in;
			}
...
//				}
		...
		public static class Stack extends TransitionEffect {
			...
			public Stack(PagedView pagedView) {
				super(pagedView, pagedView.getResources().getStringArray(R.array.transition_list)[7]);
			}
			...
			public void screenScrolled(View v, int i, float scrollProgress) {
				...
				if (v instanceof CellLayout) {
					((CellLayout) v).getChildrenLayout().setAlpha(alpha);
				} else {
					v.setAlpha(alpha);
				}

				// If the view has 0 alpha, we move it off screen so as to
				// prevent
		...
		public static class Accordion extends TransitionEffect {
			public Accordion(PagedView pagedView) {
				super(pagedView, pagedView.getResources().getStringArray(R.array.transition_list)[8]);
			}
		...
		public static class Flip extends TransitionEffect {
			public Flip(PagedView pagedView) {
				super(pagedView, pagedView.getResources().getStringArray(R.array.transition_list)[9]);
			}
		...
		public static class Cylinder extends TransitionEffect {
			...
			public Cylinder(PagedView pagedView, boolean in) {
				super(pagedView, in ? pagedView.getResources().getStringArray(R.array.transition_list)[10] : 
					pagedView.getResources().getStringArray(R.array.transition_list)[11]);
				mIn = in;
			}

			@Override
			public void screenScrolled(View v, int i, float scrollProgress) {
				float rotation = (mIn ? 42.5f : -42.5f) * scrollProgress;
				v.setCameraDistance(mPagedView.mDensity * PagedView.CAMERA_DISTANCE);
				v.setPivotX((scrollProgress + 1) * v.getMeasuredWidth() * 0.5f);// (scrollProgress +1) *
				v.setPivotY(v.getMeasuredHeight() * 0.5f);
				v.setRotationY(rotation);
			}
		}
		...
		public static class Overview extends TransitionEffect {
			...
			@Override
			public void screenScrolled(View v, int i, float scrollProgress) {
				float scale = 1.0f - 0.1f * mScaleInterpolator.getInterpolation(Math.min(0.3f, Math.abs(scrollProgress)) / 0.3f);
