...
public class DeviceProfile {
    ...
    public int iconTextSizePx;            // 文字大小
    public int iconDrawablePaddingPx;     //文字和图标间的间隔
    int cellWidthPx;
    int cellHeightPx;
    ...
    int hotseatCellWidthPx;        // Hotseat中Cell的宽度
    int hotseatCellHeightPx;       // Hotseat中Cell的高度
     ...
     * @param is 图标大小
     * @param its 图标名称大小
     * @param hs  Hotseat的图标有几个，双屏时有5个，单屏有4个
     * @param his Hotseat图标大小
     * @param dlId  单屏时的配置文件id
     * @param dnalId 双屏时的配置文件id
     */
    DeviceProfile(String n, float w, float h, float r, float c,
                  float is, float its, float hs, float his, int dlId, int dnalId) {
        // Ensure that we have an odd number of hotseat items (since we need to place all apps)
    	// Hotseat中cell数目必须是奇数，因为中间是应用抽屉的入口  
    ...
    private void updateIconSize(Context context, float scale, int drawablePadding, Resources resources,
        ...
        if (context.getResources().getBoolean(R.bool.hotseat_title_show)){
        ...
        folderCellHeightPx = (int) (cellHeightPx + 1.5f * edgeMarginPx);
        // Folder
        if (context.getResources().getInteger(R.integer.folder_type) != 1){
        	folderCellWidthPx = calculateCellWidth(widthPx, (int)numColumns);
    		folderCellHeightPx = cellHeightPx + (int) (3f * edgeMarginPx);
        }
//        folderBackgroundOffset = -edgeMarginPx;
    ...
    void calculateCellWidthWithCount(int width, int countX) {
    	folderCellWidthPx = width / 4;
    }
    ...
    public void layout(Launcher launcher) {
        ...
        ((Workspace)workspace).setOriginalTopPadding(padding.top);
        workspace.setPadding(padding.left, padding.top/2, padding.right, padding.bottom);
        ...
        // Layout the page indicators  页面指示器
