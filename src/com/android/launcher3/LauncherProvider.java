...
//            }
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
        ...
        private static final String TAG_SYSTEMAPP = "systemapp";
        private static final String TAG_CUSTOMWIDGET = "customwidget";
        ...
        private static final String ATTR_FOLDER_ID = "folderid";
        ...
        private void removeOrphanedItems(SQLiteDatabase db) {
            ...
            String removeOrphanedFolderItems = "DELETE FROM " + TABLE_FAVORITES +
                    ...
                    Favorites._ID + " FROM " + TABLE_FAVORITES +
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    if (TAG_FAVORITE.equals(name)) {
                        ...
                        if (container == Favorites.CONTAINER_HOTSEAT){
                        	id = loadSystemApp(db, values, parser);
                        } else {
                        	id = addAppShortcut(db, values, parser);
                        }
                        added = id >= 0;
                    ...
                    } else if (TAG_SYSTEMAPP.equals(name)){
                    	long id = loadSystemApp(db, values, parser);
                    	added = id >= 0;
                    } else if (TAG_CUSTOMWIDGET.equals(name)){
                    	long id = loadCustomWidget(db, values, parser);
                    	added = id >= 0;
                    }
                    if (added) {
        ...
        //解析xml中自定义widget
        private long loadCustomWidget(SQLiteDatabase db, ContentValues values,
				XmlResourceParser parser){
        	final String itemType = getAttributeValue(parser, ATTR_ITEM_TYPE);
            String title = getAttributeValue(parser, ATTR_TITLE);
        ...
        //解析xml中systemapp
        private long loadSystemApp(SQLiteDatabase db, ContentValues values,
				XmlResourceParser parser) {
        	final String packageName = getAttributeValue(parser, ATTR_PACKAGE_NAME);
            String title = null;

            if (!TextUtils.isEmpty(packageName)) {
            	final Intent intent = PackageDbUtil.getInstance(mContext).getIntentWithPkg(packageName, null);
            	List<ResolveInfo> resolveInfos = null;
            	if (intent != null){
	                try {
	                    resolveInfos = mPackageManager.queryIntentActivities(intent, 0);
	                    if (resolveInfos != null && resolveInfos.size() > 0) {
							title = resolveInfos.get(0).loadLabel(mPackageManager).toString();	
	                    }
	                    return addAppShortcut(db, values, title, intent);
	                } catch (Exception e) {
	                    Log.w(TAG, "Unable to add systemapp: " + packageName, e);
	                }	
            	}
                Log.w(TAG, "Failed to add systemapp: " + packageName);
                return -1;
            } else {
        ...
        private void addToFolder(SQLiteDatabase db, Resources res, XmlResourceParser parser,
            ...
            while ((type = parser.next()) != XmlPullParser.END_TAG ||
                ...
                childValues.put(Favorites.CONTAINER, folderId);
                ...
                } else if (TAG_SHORTCUT.equals(tag) && folderId != -1) {
                    final long id = addUriShortcut(db, childValues, res, parser);
                    if (id >= 0) {
                        folderItems.add(id);
                    }
                } else if (TAG_INCLUDE.equals(tag) && folderId != -1) {
...
//            }
            ...
            folderid = getAttributeValue(parser, ATTR_FOLDER_ID);
            values.put(Favorites.TITLE, title);
            long folderId = -1;
            if (TextUtils.isEmpty(folderid)){
            	folderId = addFolder(db, values);
            } else {
            	folderId = addFolderWithId(db, values, Long.valueOf(folderid));
            }
            boolean added = folderId != -1;
            ...
            // 文件夹内icon至少得为2个，如果少于两个，则会从数据库中删除该文件夹数据
//            if (folderItems.size() < 1 && added) {
//                // Delete the folder
...
//            }
        ...
        //用指定的文件夹id添加文件夹
        private long addFolderWithId(SQLiteDatabase db, ContentValues values, Long folderId) {
            values.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_FOLDER);
            values.put(Favorites.SPANX, 1);
            values.put(Favorites.SPANY, 1);
            values.put(Favorites._ID, folderId);
            long result = dbInsertAndCheck(this, db, TABLE_FAVORITES, null, values);
            if (result == -1) {
                return result;
            } else {
                return folderId;
                ...
                try {
                    if (c.getCount() > 0) {
                        ...
                        final int intentIndex
                                = c.getColumnIndexOrThrow(Favorites.INTENT);
                        final int titleIndex
                                = c.getColumnIndexOrThrow(Favorites.TITLE);
                        final int iconTypeIndex
                                = c.getColumnIndexOrThrow(Favorites.ICON_TYPE);
                        final int iconIndex
                                = c.getColumnIndexOrThrow(Favorites.ICON);
                        ...
                        final int containerIndex
                                = c.getColumnIndexOrThrow(Favorites.CONTAINER);
                        final int itemTypeIndex
                                = c.getColumnIndexOrThrow(Favorites.ITEM_TYPE);
                        final int screenIndex
                                = c.getColumnIndexOrThrow(Favorites.SCREEN);
                        final int cellXIndex
                                = c.getColumnIndexOrThrow(Favorites.CELLX);
                        final int cellYIndex
                                = c.getColumnIndexOrThrow(Favorites.CELLY);
                        final int uriIndex
                                = c.getColumnIndexOrThrow(Favorites.URI);
                        final int displayModeIndex
                                = c.getColumnIndexOrThrow(Favorites.DISPLAY_MODE);
                        final int profileIndex
                                = c.getColumnIndex(Favorites.PROFILE_ID);
                        ...
                        while (c.moveToNext()) {
                            ...
                            Launcher.addDumpLog(TAG, "migrating \""
                                ...
                                + cellX + "," + cellY + "@"
                                + Favorites.containerToString(container)
                                + "/" + screen
                                + "): " + intentStr, true);
                            ...
                            ContentValues values = new ContentValues(c.getColumnCount());
                            values.put(Favorites._ID, c.getInt(idIndex));
                            values.put(Favorites.INTENT, intentStr);
                            values.put(Favorites.TITLE, c.getString(titleIndex));
                            ...
                            values.put(Favorites.ICON_RESOURCE,
                                    c.getString(iconResourceIndex));
                            values.put(Favorites.ITEM_TYPE, itemType);
                            values.put(Favorites.APPWIDGET_ID, -1);
                            values.put(Favorites.URI, c.getString(uriIndex));
                            values.put(Favorites.DISPLAY_MODE,
                        ...
                        for (ContentValues values: allItems) {
                            if (values.getAsInteger(Favorites.CONTAINER) !=
                                    Favorites.CONTAINER_DESKTOP) {
                                // Hotseat items and folder items have already had their
                                // location information set. Nothing to be done here.
                                continue;
                            }
                            values.put(Favorites.SCREEN, curScreen);
                            values.put(Favorites.CELLX, curX);
                            values.put(Favorites.CELLY, curY);
                            curX = (curX + 1) % width;
                            if (curX == 0) {
                                curY = (curY + 1);
                            }
