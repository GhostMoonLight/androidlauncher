...
public class DragController {
    ...
    //注册拖拽事件监听的对象，只有WorkSpace，DeleteDropTarget和SearchDropTargetBar实现了这个监听接口
    ...
    private void checkTouchMove(DropTarget dropTarget) {
        if (dropTarget != null) {
            if (mLastDropTarget != dropTarget) {
                if (mLastDropTarget != null) {
                	//检测到与上次的dropTarget不一致，说明DragView已经滑出上一次dropTarget，
                	//就执行上一次mLastDropTarget的onDragExit方法
                    mLastDropTarget.onDragExit(mDragObject);
                }
                //检测到与上次的dropTarget不一致就执行进入另一个dropTarget的方法
                dropTarget.onDragEnter(mDragObject);
            }
            //如果前后移动的dropTarget一致，则调用当前dropTarget的onDragOver方法，
    ...
    private void drop(float x, float y) {
        ...
        if (Workspace.isShowUninstallIcon){
	        if (LauncherAppState.isDisableAllApps() && dropTarget instanceof DeleteDropTarget 
	        		&& !(mDragObject.dragInfo instanceof LauncherAppWidgetInfo)) {
	        	delete = true;
	        	if (mDragObject.dragInfo instanceof ShortcutInfo){
	        		ShortcutInfo shortcutInfo = (ShortcutInfo) mDragObject.dragInfo;
	        		if (shortcutInfo.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION){
	        			
	        			boolean canDelete = false;
						try {
							canDelete = PackageUtil.canUninstall(mLauncher, shortcutInfo);
							
							if (DeleteDropTarget.isSelfComponent(mLauncher, shortcutInfo)) {
								canDelete = false;
							}
						} catch (Exception e) {
							e.printStackTrace();
						...
						if (canDelete && !TextUtils.isEmpty(shortcutInfo.getPckName()) && PackageUtil.isInstalledApk(mLauncher, shortcutInfo.getPckName())) {
							mLauncher.getSearchBar().setVisibility(View.GONE);
							PackageUtil.uninstallApp(mLauncher, shortcutInfo.getPckName());
						} else {
							if (TextUtils.isEmpty(shortcutInfo.getPckName()) || !PackageUtil.isInstalledApk(mLauncher, shortcutInfo.getPckName())) {
								delete = false;
							}
						}
	        		}else{
        ...
        mDragObject.x = coordinates[0];
        mDragObject.y = coordinates[1];
        boolean accepted = false; //拖拽是否成功，也就是是否成功放入到一个DropTarget中
        if (dropTarget != null) {
            mDragObject.dragComplete = true;
            dropTarget.onDragExit(mDragObject);
            if (!delete && dropTarget.acceptDrop(mDragObject)) {
            	//放入相应的DragTarget中
                dropTarget.onDrop(mDragObject);
                accepted = true;
            }
        }
        //调用DragSorce的onDropCompleted方法，拖拽完成
        mDragObject.dragSource.onDropCompleted((View) dropTarget, mDragObject, false, accepted);
    }

    //找到DragView被拖到哪一个DropTarget中了
    private DropTarget findDropTarget(int x, int y, int[] dropCoordinates) {
        ...
        for (int i=count-1; i>=0; i--) {
            ...
            if (mLauncher.getResources().getInteger(R.integer.folder_type) != 1){
            	if (target instanceof FolderSingleHanded){
            		FolderSingleHanded sFolder = (FolderSingleHanded) mLauncher.getCurrentFolder();
            		if (sFolder != null){
	            		Rect rect = sFolder.getContainerRect();
	            		r.set(rect.left, rect.top, rect.right, rect.bottom);
            		}
            	}
            }
